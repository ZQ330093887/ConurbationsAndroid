...
import com.test.admin.conurbations.model.response.LyricInfo;
...
public class FloatLyricViewManager {
    ...
    public void loadLyric(Music mPlayingMusic) {
        resetLyric(SolidApplication.getInstance().getString(R.string.lyric_loading));
        if (mPlayingMusic != null) {
            mSongName = mPlayingMusic.title;
    ...
    private boolean isHome() {
        try {
            return SolidApplication.count != 0;
        } catch (Exception e) {
            e.printStackTrace();
        }
        return false;
    ...
    private static WindowManager getWindowManager() {
        if (mWindowManager == null) {
            mWindowManager = (WindowManager) SolidApplication.getInstance().getSystemService(Context.WINDOW_SERVICE);
        }
