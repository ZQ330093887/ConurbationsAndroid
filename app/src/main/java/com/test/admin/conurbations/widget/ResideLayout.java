...
public class ResideLayout extends ViewGroup {
    ...
    private void dimChildView(View v, float mag, int fadeColor) {
        ...
        if (mag > 0 && fadeColor != 0) {
            ...
            if (v.getLayerType() != v.LAYER_TYPE_HARDWARE) {
                v.setLayerType(v.LAYER_TYPE_HARDWARE, lp.dimPaint);
            }
            invalidateChildRegion(v);
        } else if (v.getLayerType() != v.LAYER_TYPE_NONE) {
            if (lp.dimPaint != null) {
    ...
    private class DisableLayerRunnable implements Runnable {
        ...
        public void run() {
            if (mChildView.getParent() == ResideLayout.this) {
                mChildView.setLayerType(mChildView.LAYER_TYPE_NONE, null);
