...
public class DouyinVideoData extends LeVideoData {
    ...
    public static DouyinVideoData fromJSONData(String jsonStr) {
        ...
        try {
            ...
            data.playCount = json.getJSONObject("statistics").optInt("share_count");
            ...
            data.formatTimeStr = DateUtils.formatTimeStr(data.createTime);
            data.formatPlayCountStr = DateUtils.formatNumber(data.playCount);
            ...
            if (!TextUtils.isEmpty(data.title)) {
                if (data.title.length() > 30) {
                    data.filterTitleStr = DateUtils.filterStrBlank(data.title.substring(0, 30) + "...");
                } else {
                    data.filterTitleStr = DateUtils.filterStrBlank(data.title);
                }
            }
            if (!TextUtils.isEmpty(data.authorName)) {
                if (data.authorName.length() > 7) {
                    data.filterUserNameStr = DateUtils.filterStrBlank(data.authorName.substring(0, 7) + "...");
                } else {
                    data.filterUserNameStr = DateUtils.filterStrBlank(data.authorName);
                }
            }
        } catch (Exception e) {
        }
