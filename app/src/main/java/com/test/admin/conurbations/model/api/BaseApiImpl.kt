...
/**
 * Created by ZQiong on 2018/5/15.
 * 调用js方法请求数据
...
object BaseApiImpl {
        ...
        when (type) {
            "QQ" -> {
                mWebView?.callHandler("api.searchQQSong", arrayOf(params)) { retValue: JSONObject ->
                    val result = gson.fromJson<SearchData.SearchSingleData>(retValue.toString(), SearchData.SearchSingleData::class.java)
     ...
     * 获取歌曲详情
     */
    fun getSongDetail(vendor: String, id: String, success: (result: SearchData.SongDetail) -> Unit, fail: (() -> Unit)? = null) {
        mWebView?.callHandler("api.getSongDetail", arrayOf<Any>(vendor, id)) { retValue: JSONObject ->
            try {
                val result = gson.fromJson<SearchData.SongDetail>(retValue.toString(), SearchData.SongDetail::class.java)
                success.invoke(result)
            } catch (e: Throwable) {
                fail?.invoke()
            }
     ...
     */
    fun getBatchSongDetail(query: String, ids: Array<String>, success: (result: SearchData.BatchSongDetail) -> Unit) {
        mWebView?.callHandler("api.getBatchSongDetail", arrayOf<Any>(query, ids)) { retValue: JSONObject ->
            val result = gson.fromJson<SearchData.BatchSongDetail>(retValue.toString(), SearchData.BatchSongDetail::class.java)
            success.invoke(result)
        }
    }
     ...
     * id，专辑ID
     */
    fun getAlbumDetail(vendor: String, id: String, success: (result: ArtistSongsData.AlbumData) -> Unit, fail: ((String) -> Unit)? = null) {
        mWebView?.callHandler("api.getAlbumDetail", arrayOf<Any>(vendor, id)) { retValue: JSONObject ->
            try {
                val result = gson.fromJson<ArtistSongsData.AlbumData>(retValue.toString(), ArtistSongsData.AlbumData::class.java)
                success.invoke(result)
            } catch (e: Throwable) {
                e.message?.let { fail?.invoke(it) }
            }
